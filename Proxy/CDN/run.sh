...
if [[ "$RELEASE_MODE" == "true" ]]; then
    EXECUTABLE="../../target/x86_64-unknown-linux-gnu/release/cdn_proxy"
else
    EXECUTABLE="../../target/x86_64-unknown-linux-gnu/debug/cdn_proxy"
fi
...
if [[ $# -gt 0 ]]; then
    ...
    exec "$EXECUTABLE" "$@" #> cdn_proxy.log 2>&1
else
    exec "$EXECUTABLE" #> cdn_proxy.log 2>&1
fi

echo "The cdn_proxy has stopped."