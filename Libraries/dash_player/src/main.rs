...
async fn main() {
    ...
    let callback = |event: dash_player::DashEvent| match event {
        dash_player::DashEvent::Segment {
            data,
            content_type,
            representation_id: _,
            segment_number,
            ...
            info!(
                "Received {} segment of size: {} at rate: {} and segment number: {}",
                content_type,
                data.len(),
                playback_rate,
                segment_number
    ...
    let player = dash_player::DashPlayer::new(mpd_url, std::sync::Arc::new(callback))
        .await
        .unwrap();
