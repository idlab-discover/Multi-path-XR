...
use super::{
    co64::Co64Box, ctts::CttsBox, generic::Mp4Box, stco::StcoBox, stsc::StscBox, stsd::StsdBox,
    stss::StssBox, stsz::StszBox, stts::SttsBox,
};
...
impl Mp4Box for StblBox {
    ...
    fn write_box(&self, buffer: &mut Vec<u8>) {
        ...
        if buffer.len() != current_size + stsd_size {
            panic!(
                "Error writing StsdBox: expected size {}, got {}",
                stsd_size,
        ...
        if buffer.len() != current_size + stts_size {
            panic!(
                "Error writing SttsBox: expected size {}, got {}",
                stts_size,
                buffer.len() - current_size
            );
        }
        if let Some(ctts) = &self.ctts {
            ...
            if buffer.len() != current_size + ctts_size {
                panic!(
                    "Error writing CttsBox: expected size {}, got {}",
                    ctts_size,
                    buffer.len() - current_size
                );
            }
        ...
        if let Some(stss) = &self.stss {
            ...
            if buffer.len() != current_size + stss_size {
                panic!(
                    "Error writing StssBox: expected size {}, got {}",
                    stss_size,
                    buffer.len() - current_size
                );
            }
        ...
        if buffer.len() != current_size + stsc_size {
            panic!(
                "Error writing StscBox: expected size {}, got {}",
                stsc_size,
                buffer.len() - current_size
            );
        }
        ...
        if buffer.len() != current_size + stsz_size {
            panic!(
                "Error writing StszBox: expected size {}, got {}",
                stsz_size,
                buffer.len() - current_size
            );
        }
        if let Some(stco) = &self.stco {
            ...
            if buffer.len() != current_size + stco_size {
                panic!(
                    "Error writing StcoBox: expected size {}, got {}",
                    stco_size,
                    buffer.len() - current_size
                );
            }
        ...
        if let Some(co64) = &self.co64 {
            ...
            if buffer.len() != current_size + co64_size {
                panic!(
                    "Error writing Co64Box: expected size {}, got {}",
                    co64_size,
                    buffer.len() - current_size
                );
            }
    ...
    fn read_box(data: &[u8]) -> Result<(Self, usize), String> {
        ...
        while offset < size {
            ...
            match box_type {
                b"stsd" => {
                    let (b, _) = StsdBox::read_box(&data[offset..])?;
                    stsd = Some(b);
                }
                b"stts" => {
                    let (b, _) = SttsBox::read_box(&data[offset..])?;
                    stts = Some(b);
                }
                b"ctts" => {
                    let (b, _) = CttsBox::read_box(&data[offset..])?;
                    ctts = Some(b);
                }
                b"stss" => {
                    let (b, _) = StssBox::read_box(&data[offset..])?;
                    stss = Some(b);
                }
                b"stsc" => {
                    let (b, _) = StscBox::read_box(&data[offset..])?;
                    stsc = Some(b);
                }
                b"stsz" => {
                    let (b, _) = StszBox::read_box(&data[offset..])?;
                    stsz = Some(b);
                }
                b"stco" => {
                    let (b, _) = StcoBox::read_box(&data[offset..])?;
                    stco = Some(b);
                }
                b"co64" => {
                    let (b, _) = Co64Box::read_box(&data[offset..])?;
                    co64 = Some(b);
                }
