...
impl Default for StsdBox {
    fn default() -> Self {
        StsdBox {
            version: 0,
            flags: 0,
            entries: vec![VisualSampleEntry::default()],
        }
    }
}
...
impl Default for VisualSampleEntry {
    fn default() -> Self {
        VisualSampleEntry {
            ...
            width: 640,
            height: 480,
            compressor_name: "SpatialCodec".to_string(),
            codec_config: None,
        }
    }
}
...
impl std::fmt::Debug for VisualSampleEntry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("VisualSampleEntry")
            ...
            .field(
                "codec_config",
                &self.codec_config.as_ref().map(|c| format_capped_bytes(c)),
            )
            .finish()
    }
}
...
impl Mp4Box for StsdBox {
    ...
    fn write_box(&self, buffer: &mut Vec<u8>) {
        ...
        for entry in &self.entries {
            ...
            if buffer.len() != current_size + entry_size {
                panic!(
                    "Error writing VisualSampleEntry: expected size {}, got {}",
                    entry_size,
    ...
    fn read_box(data: &[u8]) -> Result<(Self, usize), String> {
        ...
        for _ in 0..entry_count {
            ...
            let compressor_name =
                String::from_utf8_lossy(&data[offset + 51..compressor_name_end]).to_string();
            ...
            while sub_offset + 8 <= offset + box_size {
                let sub_box_size =
                    u32::from_be_bytes(data[sub_offset..sub_offset + 4].try_into().unwrap())
                        as usize;
