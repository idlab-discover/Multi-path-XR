...
use crate::{
    services,
    types::{AppState, ClockSyncRequest, ClockSyncResponse, WebRtcIceCandidate, WebRtcOffer},
};
use axum::{
    extract::{Query, State},
    Json,
...
#[instrument(skip_all)]
pub async fn list_sockets(State(app_state): State<AppState>) -> Json<SimpleSocketsResponse> {
...
#[instrument(skip_all)]
pub fn create_websocket_router_layer(
    ...
    io_clone.ns("/", move |socket: SocketRef| {
        let stream_manager = Arc::clone(&stream_manager);
        let server_instance_id = Arc::clone(&server_instance_id);
        async move {
            ...
            socket.on_disconnect(
                move |socket: SocketRef, reason: DisconnectReason| async move {
                    ...
                    // Clean up our data channels
                    {
                        ...
                        if let (Some(egress), Some(ingress)) = (webrtc_egress, webrtc_ingress) {
                            let client_id = socket.id.to_string();
                            egress.close_peer_connection(&client_id.clone());
            ...
            let clock_server_instance_id = Arc::clone(&server_instance_id);
            socket.on(
                "clock_sync_request",
                move |s: SocketRef, Data(request): Data<ClockSyncRequest>| {
                    let clock_server_instance_id = Arc::clone(&clock_server_instance_id);
                    async move {
                        let remote_receive_us = current_time_us();
                        let response = ClockSyncResponse {
                            server_instance_id: (*clock_server_instance_id).clone(),
                            local_send_us: request.local_send_us,
                            remote_receive_us,
                            remote_send_us: current_time_us(),
                            sequence: request.sequence,
                        };
                        if let Err(err) = s.emit("clock_sync_response", &response) {
                            debug!("Failed to emit clock sync response: {:?}", err);
                        }
                    }
                },
            );
                ...
                move |s: SocketRef, Data(candidate): Data<WebRtcIceCandidate>| {
                    ...
                    async move {
                        if let Some(webrtc_egress) = stream_manager_clone.get_webrtc_egress() {
                            ...
                            rt.spawn(async move {
                                let _ = webrtc_egress
                                    .handle_client_ice_candidate(socket_id, candidate)
                                    .await;
                            });
                        }
                    }
