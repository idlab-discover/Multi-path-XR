...
class TCIntf( Intf ):
    ...
    def bwCmds( self, bw=None, speedup=0, use_hfsc=False, use_tbf=False,
        ...
        if bw and ( bw < 0 or bw > self.bwParamMax ):
            ...
            # Improve the burst to match the bandwidth
            burst = 15000  # 15kb is a reasonable default, under normal circumstances
            if bw > 1000:
                def calculate_burst(bw_mbps, buffer_ms):
                    """
                    Calculate a recommended burst size (in bytes) for a token-bucket 
                    or HTB qdisc, given:
                    - bw_mbps: bandwidth in Mbit/s
                    - buffer_ms: desired buffer duration in milliseconds
                    Formula:
                        burst (bytes) = bw (bits/s) * buffer (s) / 8
                    """
                    bits_per_s = bw_mbps * 1_000_000    # Convert Mbit/s to bits/s
                    buffer_s = buffer_ms / 1000.0       # Convert ms to seconds
                    bits_in_buffer = bits_per_s * buffer_s
                    burst_bytes = bits_in_buffer / 8    # Convert bits to bytes
                    return int(burst_bytes)
                buffer_ms = 1.0  # ms
                burst = calculate_burst(bw, buffer_ms)
            ...
            if use_hfsc:
                ...
                cmds += [ '%s qdisc add dev %s root handle 5: tbf ' +
                          'rate %fMbit burst %d latency %fms' %
                          ( bw, burst, latency_ms ) ]
            else:
                cmds += [ '%s qdisc add dev %s root handle 5:0 htb default 1',
                          '%s class add dev %s parent 5:0 classid 5:1 htb ' +
                          'rate %fMbit burst %d' % (bw, burst) ]
            parent = ' parent 5:1 '
