...
impl VirtualWallLiteHandler {
    ...
    async fn ensure_manager(&self) -> Result<Arc<VirtualWallManager>, String> {
        ...
        let path = PathBuf::from("../Environments/VirtualWallLite/virtual_wall.private.toml");
        match VirtualWallManager::try_from_path(Some(&path)) {
            Ok(manager) => {
...
#[async_trait]
impl EnvironmentHandler for VirtualWallLiteHandler {
    async fn start(&self, options: &str) -> Result<String, String> {
        ...
        info!("Starting Virtual Wall Lite environment");
        match manager.start_from_options(parsed_options).await {
            Ok(summary) => Ok(format!(
                "Virtual Wall Lite experiment `{}` started with {} resources",
                summary.experiment_name,
                summary.resources.len()
            )),
            Err(err) => Err(format!("Failed to start Virtual Wall Lite: {err}")),
        }
    }
    ...
    async fn stop(&self) -> Result<String, String> {
        ...
        match manager.stop().await {
            Ok(_) => Ok(
                "Virtual Wall Lite environment stopped; reusable SSH state preserved"
                    .to_string(),
            ),
            Err(err) => Err(format!("Failed to stop Virtual Wall Lite: {err}")),
        }
    }
    ...
    async fn exec(&self, params: HashMap<String, String>) -> Result<String, String> {
        ...
        {
            Ok(output) => Ok(output),
            Err(err) => Err(format!("Virtual Wall Lite exec failed: {err}")),
        }
    }
    ...
    async fn start_xterm(&self, params: HashMap<String, String>) -> Result<String, String> {
        ...
        manager
            .start_terminal(&node)
            .await
            .map_err(|e| format!("Failed to open shell for Virtual Wall Lite: {e}"))
    }
